...
const {create} = require('./alaexplorerjs-fcbuffer-v2.0.0')
...
describe('Override', function () {
  ...
  it('type', function () {
    ...
    const asset = structs.asset.fromObject('1 ALA')
    assert.deepEqual(asset, {amount: 1, symbol: 'ALA'})
    assert.deepEqual('1 ALA', structs.asset.toObject(asset))
  })
...
describe('Custom Type', function () {
  it('Implied Decimal', function () {
    ...
    assert.equal(errors.length, 0)
    const asset = structs.asset.fromObject({amount: '1', symbol: 'ALA'})
    assert.deepEqual(asset, {amount: '1.0000', symbol: 'ALA'})
  })
})
